...
- [ ] **Step 1: Replace the stale Repository Structure section and add YubiKey + Bitwarden usage docs**

The current README.md has an outdated directory tree (references `config/`, `docs/`, `dotfiles/` which don't exist) and no mention of the module system, YubiKey, or Bitwarden. Replace the entire file content with an accurate, user-facing README:
...
```bash
head -5 README.md
grep "yubikey\|bitwarden" README.md
```

Expected: file starts with `# Mango Linux Configuration`, both module names appear.

- [ ] **Step 3: Commit**

```bash
git add README.md
git commit -m "docs: rewrite README with accurate module docs and YubiKey/Bitwarden usage"
```
...
- **Spec coverage:** PAM U2F ✓, SSH FIDO2 ✓, GPG signing ✓, Bitwarden install ✓, YubiKey OTP 2FA ✓, SSH key pull ✓, env file pull ✓, session cleanup ✓, MODULES.md ✓, README ✓
